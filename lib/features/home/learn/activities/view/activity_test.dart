...
class ActivityTest extends StatefulWidget {
  static const String routeName = '/activity_test';
  const ActivityTest();
...
class _WordFindWidgetState extends State<WordFindWidget> {
  ...
  void initState() {
    ...
    generatePuzzle();
    initTts();
    _speak("Welcome to the word find game");
  }
  ...
  void generatePuzzle({
    ...
    List<String> allLetters = List.from(answerLetters);

    const String alphabet = 'abcdefghijklmnopqrstuvwxyz';
    final Random random = Random();

    while (allLetters.length < 24) {
      // Changed to 24 for 6x4 grid
      allLetters.add(alphabet[random.nextInt(alphabet.length)]);
    }

    allLetters.shuffle();
    currentQues.arrayBtns = allLetters;
